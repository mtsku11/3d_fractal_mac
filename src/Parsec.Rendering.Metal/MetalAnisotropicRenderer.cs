...
public sealed class MetalAnisotropicRenderer : IDisposable
{
    ...
    public FractalGeometryStats? RunTelemetryPass(AnisotropicParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("anisotropic_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("anisotropic_telemetry"));
