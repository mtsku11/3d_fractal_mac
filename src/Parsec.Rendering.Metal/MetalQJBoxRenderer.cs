...
public sealed class MetalQJBoxRenderer : IDisposable
{
    ...
    public MetalQJBoxRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("qjbox_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("qjbox_raymarch"));
    ...
    public uint[] RenderQJBox(QJBoxParams qb, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(qb));
    ...
    private static MetalFoldParams BuildFoldParams(QJBoxParams qb) => new()
    {
        ...
        SurfParams = new Vector4(qb.Rotation, 0f),
        JuliaCVec  = qb.C,
        Rot        = new Vector4(qb.WSlice, qb.PlaneOffset, qb.CutAxisFlag, qb.Fudge),
        BoundSphere= new Vector4(0f, 0f, 0f, qb.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalQJBoxRenderer)); }
