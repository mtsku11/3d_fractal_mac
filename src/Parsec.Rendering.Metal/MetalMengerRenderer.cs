...
public sealed class MetalMengerRenderer : IDisposable
{
    ...
    public MetalMengerRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = LoadEmbeddedMsl("menger_raymarch.metal");
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("menger_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("menger_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(MengerParams mg) => new()
    {
        Iterations = mg.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        BoxParams  = new Vector4(mg.Scale, mg.Offset.X, mg.Offset.Y, mg.Offset.Z),
        SurfParams = new Vector4(mg.Rotation, 0f),
        JuliaCVec  = Vector4.Zero,
        Rot        = new Vector4(0f, 0f, 0f, mg.Fudge),
        BoundSphere= new Vector4(0f, 0f, 0f, mg.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalMengerRenderer)); }
