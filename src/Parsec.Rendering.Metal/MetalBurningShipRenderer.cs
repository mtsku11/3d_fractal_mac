...
public sealed class MetalBurningShipRenderer : IDisposable
{
    ...
    public MetalBurningShipRenderer()
    {
        try
        {
            ...
            // InjectOrbitTrap handles both triplanar and orbit-trap modes (background.w 1 vs 2).
            // The standard Inject() call would idempotently no-op after this.
            var src = MetalSurfaceTextureShaderInjector.InjectOrbitTrap(LoadEmbeddedMsl("burningship_raymarch.metal"));
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        var _hdrAcc = MetalSsaa.AccumulateHdr(settings.HeroSamples, width, height, jitter =>
        {
            ...
            var outBuf    = MetalBufferIO.CreateSharedBuffer(_device, (ulong)(width * height * 4 * sizeof(float)));
