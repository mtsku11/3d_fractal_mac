...
public sealed class MetalRotBoxRenderer : IDisposable
{
    ...
    public MetalRotBoxRenderer()
    {
        try
        {
            ...
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("rotbox_raymarch.metal"));
    ...
    // -------------------------------------------------------------------------
    // Trimmed MIDI-only telemetry pass (geometry stats + 4x4 cells + full-res
    // centroid). No sonification wavetable/orbit-trajectory/field-scan buffers.
    // -------------------------------------------------------------------------
    ...
    public FractalGeometryStats? RunTelemetryPass(
        RotBoxParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("rotbox_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("rotbox_telemetry"));
