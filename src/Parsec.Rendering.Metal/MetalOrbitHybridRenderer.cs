...
public sealed class MetalOrbitHybridRenderer : IDisposable
{
    ...
    public MetalOrbitHybridRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("orbithybrid_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("orbithybrid_raymarch"));
    ...
    public uint[] RenderOrbitHybrid(OrbitHybridParams oh, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(oh));
    ...
    public FractalGeometryStats? RunTelemetryPass(OrbitHybridParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("orbithybrid_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("orbithybrid_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(OrbitHybridParams oh) => new()
    {
        Iterations = oh.Iterations, Mode = oh.KifsCount, JuliaMode = oh.MboxCount, Pad0 = 0,
        ...
        BoundSphere= new Vector4(0f, 0f, 0f, oh.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalOrbitHybridRenderer)); }
