...
public sealed class MetalMoselyRenderer : IDisposable
{
    ...
    public MetalMoselyRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("mosely_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("mosely_raymarch"));
    ...
    public uint[] RenderMosely(MoselyParams ms, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(ms));
    ...
    public FractalGeometryStats? RunTelemetryPass(MoselyParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("mosely_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("mosely_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(MoselyParams ms) => new()
    {
        Iterations = ms.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        ...
        BoundSphere= new Vector4(0f, 0f, 0f, ms.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalMoselyRenderer)); }
