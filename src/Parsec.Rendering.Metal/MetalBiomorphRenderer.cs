...
public sealed class MetalBiomorphRenderer : IDisposable
{
    ...
    public MetalBiomorphRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("biomorph_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("biomorph_raymarch"));
    ...
    public uint[] RenderBiomorph(BiomorphParams bp, Camera3D camera, int width, int height,
    ...
    public FractalGeometryStats? RunTelemetryPass(BiomorphParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("biomorph_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("biomorph_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(BiomorphParams bp) => new()
    {
        Iterations = bp.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        BoxParams  = new Vector4(bp.Bailout, bp.PlaneOffset, 0f, bp.Cut ? 1f : 0f),
        SurfParams = new Vector4(Vector3.Normalize(bp.PlaneNormal), 0f),
        JuliaCVec  = new Vector4(bp.C, 0f),
        Rot        = new Vector4(0f, 0f, 0f, bp.Fudge),
        BoundSphere= new Vector4(0f, 0f, 0f, bp.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalBiomorphRenderer)); }
