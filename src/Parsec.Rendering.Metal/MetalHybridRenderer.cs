...
public sealed class MetalHybridRenderer : IDisposable
{
    ...
    public FractalGeometryStats? RunTelemetryPass(HybridParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("hybrid_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("hybrid_telemetry"));
