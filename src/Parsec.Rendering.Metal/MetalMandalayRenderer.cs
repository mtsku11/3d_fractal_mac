...
public sealed class MetalMandalayRenderer : IDisposable
{
    ...
    public MetalMandalayRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("mandalay_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("mandalay_raymarch"));
    ...
    public FractalGeometryStats? RunTelemetryPass(MandalayParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("mandalay_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("mandalay_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(MandalayParams md) => new()
    {
        Iterations = md.Iterations, Mode = md.Mode, JuliaMode = md.JuliaMode, Pad0 = 0,
        ...
        BoundSphere= new Vector4(0f, 0f, 0f, md.BoundRadius),
    };
