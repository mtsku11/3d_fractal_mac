...
public sealed class MetalBicomplexRenderer : IDisposable
{
    ...
    public MetalBicomplexRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("bicomplex_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("bicomplex_raymarch"));
    ...
    public uint[] RenderBicomplex(BicomplexParams bp, Camera3D camera, int width, int height,
    ...
    public FractalGeometryStats? RunTelemetryPass(BicomplexParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("bicomplex_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("bicomplex_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(BicomplexParams bp) => new()
    {
        Iterations = bp.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        BoxParams  = new Vector4(bp.XMul, bp.YMul, bp.ZMul, bp.WMul),
        SurfParams = new Vector4(bp.WAdd, bp.WSlice, 0f, 0f),
        JuliaCVec  = bp.C,
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalBicomplexRenderer)); }
