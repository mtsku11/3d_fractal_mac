...
public sealed class MetalPseudoKleinian4DRenderer : IDisposable
{
    ...
    public MetalPseudoKleinian4DRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("pseudokleinian4d_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("pseudokleinian4d_raymarch"));
    ...
    public uint[] RenderPseudoKleinian4D(PseudoKleinian4DParams pk, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(pk));
    ...
    public FractalGeometryStats? RunTelemetryPass(PseudoKleinian4DParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("pseudokleinian4d_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("pseudokleinian4d_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(PseudoKleinian4DParams pk) => new()
    {
        Iterations = pk.Iterations, Mode = pk.Mode, JuliaMode = pk.InversionMode, Pad0 = 0,
        ...
        BoundSphere= new Vector4(0f, 0f, 0f, pk.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalPseudoKleinian4DRenderer)); }
