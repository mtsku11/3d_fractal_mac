...
struct FoldParams {
    ...
    float4 surfParams;   // (rotX, rotY, rotZ, _) — Euler angles in radians
...
// ============================================================================
// RotBox (rotation-augmented Mandelbox) distance estimator
// ============================================================================
//
// Standard Mandelbox (box fold, sphere fold, scale) with a per-iteration
// 3D rotation applied before the folds. The three Euler angles compound
// across iterations, making them extremely sensitive generative knobs.
//
// boxParams: (scale, minRadius, fixedRadius, foldLimit)
// surfParams.xyz: Euler angles (radians) applied each iteration
...
float3x3 eulerRotation(float ax, float ay, float az) {
    ...
    // float3x3(col0, col1, col2) — column-major, matches GLSL mat3 column-major fill
