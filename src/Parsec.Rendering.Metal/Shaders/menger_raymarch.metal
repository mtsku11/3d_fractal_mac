...
Hit traceRay(float3 ro, float3 rd,
    ...
    h.normal = estimateNormal(hitPoint, nEps, rd, degenerate, fp);
...
float3 shadeDirect(Hit h, float hitEps, float maxDist,
    ...
    if (softShadowsOn && lambert > 0.0f)
        shadow = softShadow(off, rp.lightDir.xyz, hitEps, maxDist, rp.marchI1, rp.marchA.w, fp);
    float ao = 1.0f;
    if (aoOn) ao = ambientOcclusion(off, h.normal, rp.marchB.x, rp.marchB.y, rp.marchI2, fp);
    float intensity = rp.lightDir.w <= 0.0f ? 1.0f : rp.lightDir.w;
    float lighting = 0.25f * ao + 0.75f * lambert * shadow * ao * intensity;
    return h.albedo * clamp(lighting, 0.0f, 1.0f);
}

kernel void menger_raymarch(
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    uint3 q = uint3(clamp(color, float3(0), float3(1)) * 255.0f + 0.5f);
