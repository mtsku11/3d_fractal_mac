...
struct FoldParams {
    ...
    float4 boxParams;  // (p_mem, planeOffset, bailout, cutEnabled)
...
float3 phxBulbPow2(float3 p) {
    float r2 = dot(p, p), r = sqrt(r2);
    float theta = asin(clamp(p.z / max(r, 1e-12f), -1.0f, 1.0f));
    float phi   = atan2(p.y, p.x);
    float cosTh = cos(2*theta);
    return float3(r2*cosTh*cos(2*phi), r2*cosTh*sin(2*phi), r2*sin(2*theta));
}
...
float estimateFull(float3 p, constant FoldParams& fp, thread float4& outTrap) {
    ...
    float  bailout    = max(fp.boxParams.z, 2.0f);
    bool   cutEnabled = fp.boxParams.w > 0.5f;
    float  bo2 = bailout * bailout;

    float3 z = p, zPrev = float3(0);
    float dz = 1.0f, dzPrev = 0.0f;
    outTrap = float4(1e20f);

    for (int i = 0; i < fp.iterations; i++) {
        if (dot(z, z) > bo2) break;
        float r = length(z);
        float dzNew = min(2.0f * r * dz + abs(pMem) * dzPrev, 1e30f);
        float3 zNew = phxBulbPow2(z) + c + pMem * zPrev;
        zPrev = z; z = zNew; dzPrev = dz; dz = dzNew;
