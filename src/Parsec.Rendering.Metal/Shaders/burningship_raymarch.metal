...
float estimateFull(float3 p, constant FoldParams& fp, thread float4& outTrap) {
    ...
    for (int i = 0; i < fp.iterations; i++) {
        ...
        float theta  = atan2(sq_xz, z.y);
        float zangle = atan2(z.x, z.z);
        ...
        theta  *= power;
        zangle *= power;
        z = zr * float3(sin(zangle) * sin(theta),
                        cos(theta),
...
float3 shadeDirect(Hit h, float hitEps, float maxDist,
                   constant FoldParams& fp, constant RenderParams& rp) {
    float shadowSoft = rp.marchA.w; float aoStep = rp.marchB.x; float aoIntensity = rp.marchB.y;
    int shadowSteps = rp.marchI1; int aoSamples = rp.marchI2; int flags = rp.marchI3;
    bool softShadowsOn = (flags & 1) != 0; bool aoOn = (flags & 2) != 0;
    ...
    if (softShadowsOn && lambert > 0.0f)
        shadow = softShadow(offsetPoint, rp.lightDir.xyz, hitEps, maxDist, shadowSteps, shadowSoft, fp);
    float ao = 1.0f;
    if (aoOn) ao = ambientOcclusion(offsetPoint, h.normal, aoStep, aoIntensity, aoSamples, fp);
    float intensity = rp.lightDir.w <= 0.0f ? 1.0f : rp.lightDir.w;
...
kernel void burningship_raymarch(
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    float hitEps = rp.marchA.x; float maxDist = rp.marchA.y;
    float normalEps = rp.marchA.z; int maxSteps = rp.marchI0;
