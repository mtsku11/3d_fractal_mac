...
public sealed class MetalQuaternionJuliaRenderer : IDisposable
{
    ...
    public MetalQuaternionJuliaRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("quaternianjulia_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("quaternianjulia_raymarch"));
    ...
    public uint[] RenderQuaternionJulia(QuaternionJuliaParams qj, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(qj));
    ...
    public FractalGeometryStats? RunTelemetryPass(QuaternionJuliaParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("quaternianjulia_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("quaternianjulia_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(QuaternionJuliaParams qj) => new()
    {
        Iterations = qj.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        BoxParams  = new Vector4(qj.WSlice, qj.PlaneOffset, qj.Bailout, qj.Cut ? 1f : 0f),
        SurfParams = new Vector4(Vector3.Normalize(qj.PlaneNormal), 0f),
        JuliaCVec  = qj.C,
        Rot        = new Vector4(qj.Stereo ? 1f : 0f, qj.StereoK, qj.StereoR, qj.Fudge),
        // Disable bounding sphere in stereographic mode (wraps all of R^3 onto the sphere)
        BoundSphere= new Vector4(0f, 0f, 0f, qj.Stereo ? 1e6f : qj.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalQuaternionJuliaRenderer)); }
