...
public sealed class MetalPhoenixRenderer : IDisposable
{
    ...
    public MetalPhoenixRenderer()
    {
        try {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = MetalSurfaceTextureShaderInjector.Inject(LoadEmbeddedMsl("phoenix_raymarch.metal"));
            NSError le = default; var lib = dev.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref le);
            var fn = lib.NewFunction(NSString.String("phoenix_raymarch"));
    ...
    public uint[] RenderPhoenix(PhoenixParams ph, Camera3D camera, int width, int height,
        ...
        PostProcessParams? postProcess = null)
    {
        ...
        {
            using var fb = UploadStruct(_device, BuildFoldParams(ph));
    ...
    public FractalGeometryStats? RunTelemetryPass(PhoenixParams fractal, Camera3D camera, RaymarchSettings settings)
    {
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("phoenix_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("phoenix_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(PhoenixParams ph) => new()
    {
        Iterations = ph.Iterations, Mode = 0, JuliaMode = 0, Pad0 = 0,
        BoxParams  = new Vector4(ph.PMem, ph.PlaneOffset, ph.Bailout, ph.Cut ? 1f : 0f),
        SurfParams = new Vector4(Vector3.Normalize(ph.PlaneNormal), 0f),
        JuliaCVec  = new Vector4(ph.C, 0f),
        Rot        = new Vector4(0f, 0f, 0f, ph.Fudge),
        BoundSphere= new Vector4(0f, 0f, 0f, ph.BoundRadius),
    };
    ...
    private void ThrowIfDisposed() { if(_disposed) throw new ObjectDisposedException(nameof(MetalPhoenixRenderer)); }
