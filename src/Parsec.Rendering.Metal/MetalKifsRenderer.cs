...
/// <summary>
/// Metal compute backend for KIFS (Kaleidoscopic IFS / Amazing IFS) raymarching.
/// Compiles kifs_raymarch.metal at construction. Returns packed RGBA8 uint[].
/// </summary>
public sealed class MetalKifsRenderer : IDisposable
{
    ...
    public MetalKifsRenderer()
    {
        try
        {
            var dev = MTLDevice.CreateSystemDefaultDevice();
            var src = LoadEmbeddedMsl("kifs_raymarch.metal");
    ...
    private bool EnsureTelemetryPso()
    {
        ...
        try
        {
            var src = LoadEmbeddedMsl("kifs_telemetry.metal");
            NSError libErr = default;
            var library  = _device.NewLibrary(NSString.String(src), new MTLCompileOptions(), ref libErr);
            var function = library.NewFunction(NSString.String("kifs_telemetry"));
    ...
    private static MetalFoldParams BuildFoldParams(KifsParams kf) => new()
    {
        Iterations  = kf.Iterations,
        Mode        = 0,
        JuliaMode   = 0,
        Pad0        = 0,
        // boxParams: (scale, _, minRadius, fixedRadius) — matches kifs_core.glsl
        ...
        BoundSphere = new Vector4(0f, 0f, 0f, kf.BoundRadius),
    };
    ...
    {
        if (_disposed) throw new ObjectDisposedException(nameof(MetalKifsRenderer));
    }
