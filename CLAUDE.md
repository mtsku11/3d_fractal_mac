...
- **MIDI output (geometry → MIDI).** Emits MIDI to external DAWs/VSTs (virtual CoreMIDI source "Parsec") so the fractal is an A/V control surface; namespace `Parsec.Audio.Midi`. M1–M4 done + on `main`: 15 continuous CCs (20–34), gesture notes (60/62/64/65/67/69/71), 16 spatial object-notes (36–51), all channel 1. Planned-improvement progress: real on-screen colour → CC24 hue/CC35 sat/CC36 bright (**done**), responsiveness/smoothing slider (**done**), full-res energy centroid → CC30/31/32 (**done**), MIDI-only telemetry for all fractals (**done — 20/20**, only Attractor excluded: no Metal renderer). Batch-3 kernels were generated by `tools/build_midi_telemetry.py` + `tools/add_telemetry_pass.py`; validate with the `metal-midi-telemetry` CLI. Remaining optional: 2b finer region-note grid, M3b mapping editor (both deferred). Spec in `docs/midi-output-plan.md`.
