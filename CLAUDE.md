...
- **Attractor — DONE (Metal renderer + telemetry, verified 2026-06-15).** `MetalAttractorRenderer` (`attractor_raymarch.metal`) renders the Thomas attractor on macOS: it isn't a closed-form DE — `SetAttractor(AttractorHash)` uploads the trajectory/hash/sorted-index data to Metal buffers (3/4/5) and the kernel walks the spatial hash. Wired in `FractalView` (`RenderWithMetalAttractor`, `HasMetalPreviewRenderer` true when the hash is built). CLI `metal-attractor-smoke` passes (50k-pt trajectory, ~21 ms compute, 25970/57600 non-bg px). Telemetry: `attractor_telemetry.metal` + `RunTelemetryPass` reuse the same data buffers → Attractor now emits spatial MIDI/sonification too (`metal-midi-telemetry`: hitClose 0.092, 16 cells, PASS). All 20 analytic fractals **plus Attractor** now render and sonify via Metal. (Octonion/Triball/IFS have OpenGL renderers but aren't in the `FractalType` dropdown.)
